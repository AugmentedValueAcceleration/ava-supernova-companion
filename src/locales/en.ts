/** Companion English strings — source of truth. All other locales mirror this key set. */
export const enStrings = {
  'chat': 'Chat',
  'tasks': 'Tasks',
  'journal': 'Journal',
  'settings': 'Settings',
  'newChat': 'New chat',
  'chatHistory': 'Chat History',
  'messagePlaceholder': 'Message Ava...',
  'noConversations': 'No conversations yet',
  'startChatting': 'Start chatting with Ava',
  'account': 'ACCOUNT',
  'guest': 'Guest',
  'guestSubtitle': 'Your data stays on this device',
  'signIn': 'Sign In / Create Account',
  'signOut': 'Sign Out',
  'viewPlans': 'View plans & pricing',
  'upgradePlan': 'Upgrade Plan',
  'disconnectKey': 'Disconnect API Key',
  'model': 'MODEL',
  'defaultModel': 'Default model',
  'appearance': 'APPEARANCE',
  'theme': 'Theme',
  'textSize': 'Text size',
  'dark': 'Dark',
  'light': 'Light',
  'system': 'System',
  'data': 'DATA',
  'clearChat': 'Clear chat history',
  'clearTasks': 'Clear local tasks',
  'clearJournal': 'Clear local journal',
  'areYouSure': 'Are you sure?',
  'clear': 'Clear',
  'cancel': 'Cancel',
  'about': 'ABOUT',
  'version': 'Version',
  'website': 'Website',
  'termsOfService': 'Terms of Service',
  'privacyPolicy': 'Privacy Policy',
  'localOnly': 'Local only — stays on this device',
  'synced': 'Synced',
  'memory': 'Memory',
  'addTask': 'Add task',
  'taskTitle': 'Task title',
  'noTasks': 'No tasks yet',
  'today': 'Today',
  'all': 'All',
  'overdue': 'Overdue',
  'completed': 'Completed',
  'yourJournal': 'Your Journal',
  'avasJournal': 'Ava\'s Journal',
  'writeEntry': 'Write Entry',
  'noEntry': 'No entry for this day',
  'avaNoEntry': 'Ava hasn\'t written anything for this day',
  'avaWrites': 'Ava writes her thoughts at the end of sessions',
  'saveEntry': 'Save Entry',
  'mood': 'Mood',
  'howAreYou': 'How are you feeling? What happened today? Write freely...',
  'welcome': 'Welcome to Ava',
  'welcomeSubtitle': 'Your AI companion, everywhere',
  'getStarted': 'Get Started',
  'skip': 'Skip',
  'language': 'Language',
  'delete': 'Delete',
  'deleteConfirm': 'Delete this?',
  'tapToEdit': 'Tap to edit',
  'nothingToday': 'Nothing for today',
  'noActiveTasks': 'No active tasks',
  'addTaskHint': 'Add a task above or ask Ava',
  'learning': 'Learning',
  'noLearningPaths': 'No learning paths yet',
  'askAvaToTeach': 'Ask Ava to teach you something!',
  'usage': 'USAGE',
  'tokensUsed': 'Tokens used',
  'requestsUsed': 'Requests used',
  'syncConversations': 'Sync Conversations',
  'releaseNotes': 'Release Notes',
  'confirmSignOut': 'Sign Out',
  'confirmSignOutMsg': 'Are you sure you want to sign out? Local data will remain on this device.',
  'confirmDelete': 'Delete Conversation',
  'confirmDeleteMsg': 'This conversation will be permanently deleted.',
  'offlineQueued': 'You\'re offline. Message queued.',
  'notificationsEnabled': 'Task notifications enabled',
  'personality': 'Personality',
  'designYourAI': 'Design Your AI',
  'designSubtitle': 'Make it yours',
  'resetToDefault': 'Reset to Default',
  'saved': 'Saved!',

  // Tasks + Journal body strings (the headers were already covered).
  'loading': 'Loading...',
  'addTaskPlaceholder': 'Add a task...',
  'deleteTask': 'Delete task',
  'deleteEntry': 'Delete entry',

  // Privacy / Data Mode (local-first framing — Local is default).
  'privacy': 'Privacy',
  'dataMode': 'Data Mode',
  'dataModeLocal': 'Local',
  'dataModeCloud': 'Cloud',
  'dataModeLocalDesc': 'On-device only — nothing leaves this companion.',
  'dataModeCloudDesc': 'Local-first with cloud sync — your data on every Ava surface.',

  // More menu + model picker + BYOK
  'support': 'Support',
  'supportSubtitle': 'Chat with Ava and the team',
  // ── App chrome — bottom nav + More sheet ────────────────────────────────
  'navWellbeing': 'Wellbeing',
  'navMore': 'More',
  'navNews': 'News',
  'navHelpDocs': 'Help & Docs',
  // ── Settings: Local data + About ────────────────────────────────────────
  'localData': 'Local data',
  'localDataDesc': 'Everything Ava stores on this device. Nothing leaves it.',
  'localDataShow': 'Show',
  'refresh': 'Refresh',
  'comingSoon': 'Coming soon',
  'aboutDataPromise': 'Your data stays yours. Local-first. No telemetry.',
  'aboutOpenSource': 'Open source — Apache 2.0',
  // ── Help & Docs ─────────────────────────────────────────────────────────
  // New-chat opening message. Was hardcoded English in useChat.ts, so Ava
  // greeted every user in English no matter what language they had chosen —
  // her replies followed the picker but her first words did not.
  'greetingGuest': 'Hey! I’m Ava. To start chatting, either sign in for the free tier (300 credits a month) or drop your own API key into Settings — Kimi, DeepSeek, Claude, GLM, Mistral all work.\n\nTell me what you’re up to.',
  'greetingUser': 'Hey {name}! I’m Ava — your companion on the go. I can manage your tasks, write journal entries, and chat about anything.\n\nWhat’s on your mind?',
  // Shown on machine-translated articles. The reader is entitled to know these
  // are not the source's own words, with the original one tap away.
  'newsTranslatedNotice': 'Translated automatically',
  'newsViewOriginal': 'View original',
  'connected': 'Connected',
  'connectedViaApiKey': 'Connected via API key',
  'docsShowDetails': 'Show me the details',
  'docsTitle': 'Help & Docs',
  'docsAskPlaceholder': 'Ask Ava anything…',
  'docsAskButton': 'Ask',
  'docsSearchPlaceholder': 'Or search the docs…',
  'tryAgain': 'Try again',
  // ── Personality designer ────────────────────────────────────────────────
  // Section headings. The option `value` stays English (it is persisted and
  // sent to the model); only the label and description are translated.
  'personaTitle': 'Tune Ava’s style',
  'personaSubtitle': 'Adjust how Ava communicates with you',
  'personaDescPlaceholder': 'e.g. Like a patient older brother who’s been coding for 20 years',
  'personaDescHint': 'Optional. Describe the vibe in your own words and your AI will embody it.',
  'personaPreview': 'Preview',
  'personaPreviewLine': 'Ava will be {tone}, {energy}, and {style}.',
  'personaSave': 'Save personality',
  'personaTone': 'Tone',
  'personaEnergy': 'Energy',
  'personaStyle': 'Communication style',
  'personaDescription': 'Description',
  'persona.tone.warm.label': 'Warm',
  'persona.tone.warm.desc': 'Warm and encouraging',
  'persona.tone.direct.label': 'Direct',
  'persona.tone.direct.desc': 'Direct and no-nonsense',
  'persona.tone.playful.label': 'Playful',
  'persona.tone.playful.desc': 'Playful and witty',
  'persona.tone.professional.label': 'Professional',
  'persona.tone.professional.desc': 'Professional and polished',
  'persona.tone.dry-wit.label': 'Dry Wit',
  'persona.tone.dry-wit.desc': 'Dry wit — understated brilliance',
  'persona.energy.calm.label': 'Calm',
  'persona.energy.calm.desc': 'Calm and steady',
  'persona.energy.enthusiastic.label': 'Enthusiastic',
  'persona.energy.enthusiastic.desc': 'Enthusiastic and excited',
  'persona.energy.measured.label': 'Measured',
  'persona.energy.measured.desc': 'Measured and deliberate',
  'persona.energy.excitable.label': 'Excitable',
  'persona.energy.excitable.desc': 'High energy and expressive',
  'persona.style.concise.label': 'Concise',
  'persona.style.concise.desc': 'Sharp, no filler',
  'persona.style.detailed.label': 'Detailed',
  'persona.style.detailed.desc': 'Thorough, explains the why',
  'persona.style.conversational.label': 'Conversational',
  'persona.style.conversational.desc': 'Natural, talks like a person',
  'persona.style.structured.label': 'Structured',
  'persona.style.structured.desc': 'Headers, bullets, organised',
  // ── Catalogue filters (recipes / workouts) ──────────────────────────────
  'filterCollections': 'Collections',
  'filterDiet': 'Diet',
  'filterDietary': 'Dietary',
  'filterCuisine': 'Cuisine',
  'filterTime': 'Time',
  'filterSort': 'Sort',
  'proTip': 'Pro tip',
  'pwaInstallHint': 'Add Ava to your home screen for one-tap access. iPhone: Share → Add to Home Screen. Android: menu → Install app.',
  'modelPickerSignInHint': 'Sign in for free tier, or add an API key below',
  'modelPickerSelect': 'Select a model...',
  'byokTitle': 'API KEYS (BYOK)',
  'byokDescription': 'Add your own API keys to use any model without a platform account. Keys are stored locally on your device.',

  // ── Wellbeing surface ──────────────────────────────────────────────────
  // Tabs
  'wellbeingTabToday': 'Today',
  'wellbeingTabProfile': 'Profile',

  // ── Gym ───────────────────────────────────────────────────────────────
  'gymHeading': 'Gym',
  'gymDoneTodayBadge': 'Done today',
  'gymModeStopwatch': 'Stopwatch',
  'gymModeCountdown': 'Countdown',
  'gymModeSets': 'Sets',
  'gymModeTabata': 'Tabata',
  'gymParamRounds': 'Rounds',
  'gymParamWork': 'Work',
  'gymParamRest': 'Rest',
  'gymTabataPresetLabel': '20s work · 10s rest · 8 rounds · 4:00 total',
  'gymTimerHintStart': 'Tap to start',
  'gymTimerHintPause': 'Tap to pause',
  'gymTimerHintResume': 'Tap to resume',
  'gymRoundLabel': 'Round',
  'gymButtonReset': 'Reset',
  'gymButtonFinished': 'Finished',
  'gymPaletteReady': 'Ready',
  'gymPalettePaused': 'Paused',
  'gymPaletteRest': 'Rest',
  'gymPaletteWorking': 'Working out',
  'gymFinishedOverlayBadge': 'Finished',
  'gymFinishedSessionLength': 'session length',
  'gymFinishedTickOffPrompt': 'Tick off what you did',
  'gymFinishedSaveButton': 'Save & done',

  // ── Today ─────────────────────────────────────────────────────────────
  'todayProfileSetupHint': 'Set up your Profile (tap ✦ → Profile) so Ava can plan around you.',
  'todayMorningBriefLabel': "Ava's brief",
  'todayNoBriefEmpty': "No brief yet — Ava can write one from where you're at today.",
  'todayBriefGeneratingButton': 'Writing…',
  'todayBriefRewriteButton': 'Rewrite brief',
  'todayBriefCreateButton': 'Write my brief',
  'todayStatusLabel': 'Where you are',
  'todayReadinessTile': 'Readiness',
  'todayNutritionTile': 'Nutrition',
  'todayTrainingTile': 'Training',
  'todayQuickLogLabel': 'Quick log',
  'todayPlanLabel': 'Today\'s plan',
  'todayPlanRest': 'Rest day',
  'todayPlanRecovery': 'Active recovery',
  'todayPlanTrainingLabel': 'Training',
  'todayPlanMealsLabel': 'Meals',
  'todayPlanDone': 'Done',
  'todayPlanInProgress': 'In progress',
  'todayPlanEaten': 'Eaten',
  'todayPlanSwapped': 'Swapped',
  'todayPlanSkipped': 'Skipped',
  'todayPlanRestHint': 'Nothing scheduled — recovery is part of the plan.',
  'todayPlanExtraMeals': 'Also logged',
  'mealSheetEat': 'Ate it',
  'mealSheetSkip': 'Skip',
  'mealSheetSwap': 'Swap',
  'mealSheetUndo': 'Undo',
  'mealSheetServings': 'Servings',
  'mealSheetPerServing': 'per serving',
  'mealSheetTotal': 'Total',
  'mealSheetProtein': 'Protein',
  'mealSheetCarbs': 'Carbs',
  'mealSheetFat': 'Fat',
  'progressTab': 'Progress',
  'progressAdherence': 'Adherence',
  'progressStreak': 'Streak',
  'progressSessions': 'Sessions',
  'progressDays': 'days',
  'progressLifts': 'Lifts',
  'progressIntake': 'Intake',
  'progressWeight': 'Weight',
  'progressEmpty': 'Nothing to show yet — train, eat and log, and this fills in.',
  'progressNoLifts': 'Log a lift twice and its progress appears here.',
  'progressEmptyTitle': 'Your progress builds here',
  'progressPlanDay': 'Day',
  'todayReadinessEmptyHint': 'Log sleep + mood',
  'todayReadinessStrong': 'Strong',
  'todayReadinessGood': 'Good',
  'todayReadinessFair': 'Fair',
  'todayReadinessLow': 'Low',
  'todayReadinessSleepLabel': 'sleep',
  'todayReadinessMoodLabel': 'mood',
  'todayNutritionEmptyHint': 'Log a meal',
  'todayNutritionWaterLabel': 'water',
  'todayNutritionMealsValue': 'meals',
  'todayTrainingRestValue': 'Rest',
  'todayTrainingNoSessionHint': 'No session planned',
  'todayTrainingSessionsHint': 'sessions done',
  'todayLogMealsButton': 'meals',
  'todayLogAddMealButton': 'Add meal',
  'todayLogWaterButton': 'Water',
  'todayLogSleepButton': 'Sleep',
  'todayLogMoodButton': 'Mood',
  'todayLogWaterReset': 'reset',
  'todayLogSleepClear': 'clear',
  'todayLogMealPlaceholder': 'What did you eat?',
  'todayLogMealKcalPlaceholder': 'kcal',
  'todayLogMealProteinPlaceholder': 'protein g',
  'todayLogMealAddButton': 'Add',

  // ── Profile ───────────────────────────────────────────────────────────
  'profileHeading': 'Profile',
  'profileDescription': 'What Ava plans your day around.',
  // Profile — the structured half. These fields existed on the type, synced,
  // and drove generation, with nowhere to type them in.
  'profileKitchenSection': 'Kitchen',
  'profileHouseholdLabel': 'People eating',
  'profileCookingLevelLabel': 'Cooking confidence',
  'profileMinutesWeekdayLabel': 'Minutes on a weekday',
  'profileMinutesWeekendLabel': 'Minutes at the weekend',
  'profileLevelBeginner': 'Beginner',
  'profileLevelIntermediate': 'Intermediate',
  'profileLevelExpert': 'Expert',
  'profileLevelAdvanced': 'Advanced',
  'profileTrainingSection': 'Training',
  'profileExperienceLabel': 'Experience',
  'profileDaysPerWeekLabel': 'Days per week',
  'profileTrainingDaysLabel': 'Which days',
  'profileDay_mon': 'Mon',
  'profileDay_tue': 'Tue',
  'profileDay_wed': 'Wed',
  'profileDay_thu': 'Thu',
  'profileDay_fri': 'Fri',
  'profileDay_sat': 'Sat',
  'profileDay_sun': 'Sun',
  'profileConstraintsSection': 'Constraints',
  'profileAllergensLabel': 'Allergens',
  'profileAllergensHint': 'Recipes containing these are removed before Ava ever sees them.',
  'profileDietsLabel': 'Diet',
  'profileInjuriesLabel': 'Injuries and conditions',
  'profileInjuriesHint': 'Exercises that would aggravate these are held back or modified.',
  'profileEquipmentLabel': 'Equipment you have',
  'profileEquipmentHint': 'Leave empty if you would rather not filter by kit.',

  'profileBodySection': 'Body',
  'profileSexLabel': 'Sex',
  'profileSexFemale': 'Female',
  'profileSexMale': 'Male',
  'profileSexOther': 'Other',
  'profileDobLabel': 'Date of birth',
  'profileHeightLabel': 'Height',
  'profileWeightLabel': 'Weight',
  'profileBodyFatLabel': 'Body fat % (optional)',
  'profileGoalSection': 'Goal',
  'profilePrimaryGoalLabel': 'Primary goal',
  'profileWeeklyFocusLabel': "This week's focus",
  'profileWeeklyFocusPlaceholder': 'e.g. deload week',
  'profileSleepTargetSection': 'Sleep target',
  'profileBedtimeLabel': 'Bedtime',
  'profileWakeLabel': 'Wake',
  'profileSaveButtonSaved': 'Saved ✓',
  'profileSaveButtonDefault': 'Save profile',

  // ── Plans (library + calendar) ────────────────────────────────────────
  'plansGoalFatLoss': 'Fat loss',
  'plansGoalMuscleGain': 'Muscle gain',
  'plansGoalMaintenance': 'Maintenance',
  'plansGoalAthletic': 'Athletic',
  'plansGoalRecovery': 'Recovery',
  'plansGoalLongevity': 'Longevity',
  'plansHeading': 'Plans',
  'plansNewButton': '+ New',
  'plansTabPrograms': 'Programs',
  'plansTabCalendar': 'Calendar',
  'plansEmptyState': 'No plans yet.',
  'plansStatusDraft': 'draft',
  'plansStatusActive': 'active',
  'plansStatusCompleted': 'completed',
  'plansStatusArchived': 'archived',
  'plansRepeatButton': 'Repeat',
  'plansDeleteButton': 'Delete',
  'plansCalendarLegendTraining': 'Training',
  'plansCalendarLegendMeals': 'Meals',
  'plansCalendarTapDayHint': 'Tap a day to open its plan',
  'plansCalendarEmptyState': 'No plans yet. Ask Ava for one, or build your own — both up top.',
  // Shown when plans EXIST but none has started. Telling someone who has just
  // made a plan to "create one" is both wrong and a dead end.
  'plansCalendarNotStarted': 'Nothing running yet. A plan only lands on the calendar once it starts.',
  'plansCalendarLegendProposed': 'Not started',
  'plansCalendarStartIt': 'Start',
  // The card says what the plan IS. It used to read "Tap to build" — authoring
  // language on a card for a plan that is already built — and told you nothing
  // about what was inside, so a finished week and an empty shell looked alike.
  'plansCardSessions': 'sessions',
  'plansCardMeals': 'meals',
  'plansCardKcalPerDay': 'kcal a day',
  'plansCardEmpty': 'Nothing in it yet',
  'plansCardEmptyHint': 'Add some days before starting it, or ask Ava to fill it in.',
  'plansCardStartButton': 'Start it',
  'plansCreateSheetHeading': 'New plan',
  'plansCreateTypeLabel': 'Type',
  'plansCreateDurationLabel': 'Duration',
  'plansCreateTitleLabel': 'Title',
  'plansCreateMealPlanPlaceholder': 'e.g. 4-week cut',
  'plansCreateFitnessPlanPlaceholder': 'First 5k',
  'plansCreateActivateCheckbox': 'Start now (activate) — archives any active plan of the same type',
  'plansCreateButton': 'Create plan',

  // ── Plan Builder ──────────────────────────────────────────────────────
  // Prep plan — what the week costs you in evenings.
  'prepTitle': 'Prep plan',
  'prepNothingToCook': 'Nothing to cook in this plan yet.',
  'prepAcrossThePlan': 'of cooking across the plan',
  'prepCookOnce': 'Cook once, eat again',
  'prepSaves': 'saves',
  'prepByDay': 'Where the work lands',
  'prepCookOnDay': 'Cook on day',
  'prepPortions': 'portions',
  'prepCovers': 'covers',
  'prepDayWord': 'Day',
  'prepDaysWord': 'days',
  'prepKeeps': 'Keeps',
  'prepStartAhead': 'Start ahead',
  'prepNightBefore': 'the night before',

  // Shopping list — a meal plan you cannot shop for is only a suggestion.
  'shoppingListTitle': 'Shopping list',
  'shoppingListLooking': 'Looking up what these meals need…',
  'shoppingListNoMeals': 'No meals in this plan yet.',
  'shoppingListNoMealsWeek': 'Nothing planned to cook this week.',
  'shoppingListShopWeek': 'Shop for this week',
  'shoppingListThisWeek': 'This week',
  'shoppingListBackToThisWeek': 'Back to this week',
  'shoppingListWeek': 'Week',
  'shoppingListMeals': 'meals',
  'shoppingListHideOptional': 'Hide optional',
  'shoppingListReset': 'Start again',
  'shoppingListIncomplete': 'Ingredients could not be found for these meals, so the list is short by whatever they need:',
  'shoppingListOffline': 'The library could not be reached, so these meals are not on the list yet:',
  'shoppingListRetry': 'Try again',
  'shoppingListNotInLibrary': 'These are not library recipes, so no ingredient list exists for them. Swap them for a recipe to shop for them:',
  'shoppingListToTaste': 'to taste',
  'shoppingListOptional': 'optional',
  'shoppingAisleProduce': 'Fruit & veg',
  'shoppingAisleMeatFish': 'Meat & fish',
  'shoppingAisleDairyEggs': 'Dairy & eggs',
  'shoppingAisleBakery': 'Bakery',
  'shoppingAisleCupboard': 'Store cupboard',
  'shoppingAisleSpices': 'Herbs & spices',
  'shoppingAisleTinsJars': 'Tins, jars & sauces',
  'shoppingAisleOilsVinegars': 'Oils & vinegars',
  'shoppingAisleFrozen': 'Frozen',
  'shoppingAisleOther': 'Anything else',

  'planBuilderBackTitle': 'Plan',
  'planBuilderNotFound': 'Plan not found.',
  'planBuilderWeekLabel': 'Week',
  'planBuilderTrainingKind': 'Training',
  'planBuilderRecoveryKind': 'Recovery',
  'planBuilderRestKind': 'Rest',
  'planBuilderDayTitlePlaceholder': 'Day title — e.g. Upper body, Long run',
  'planBuilderTrainingSection': 'Training',
  'planBuilderNoExercises': 'No exercises yet.',
  'planBuilderMealsSection': 'Meals',
  'planBuilderNoMeals': 'No meals yet.',
  'planBuilderNotesLabel': 'Notes',
  'planBuilderGroupAddButton': '+ Add',
  'planBuilderExerciseSetsLabel': 'sets',
  'planBuilderExerciseRepsLabel': 'reps',
  'planBuilderExerciseWeightLabel': 'weight',
  'planBuilderExerciseRestLabel': 'rest s',
  'planBuilderMealBreakfast': 'breakfast',
  'planBuilderMealLunch': 'lunch',
  'planBuilderMealDinner': 'dinner',
  'planBuilderMealSnack': 'snack',
  'planBuilderMealServingsLabel': 'servings',
  'planBuilderPickerExercisePlaceholder': 'Add a workout',
  'planBuilderPickerRecipePlaceholder': 'Add a recipe',
  'planBuilderGroupAdding': 'Adding…',

  // Builder advisories. Written to be read by someone mid-task, so each one is
  // a short statement of fact rather than an instruction — the builder tells
  // you what it noticed and leaves the decision with you.
  'planBuilderNutritionSection': 'Nutrition',
  'planBuilderCookingTotal': 'min cooking',
  'planBuilderKcalOf': 'kcal of',
  'planBuilderKcalPlanned': 'kcal planned',
  'planBuilderMacroProtein': 'Protein',
  'planBuilderMacroCarbs': 'Carbs',
  'planBuilderMacroFat': 'Fat',
  'planBuilderNoTargetYet': 'No calorie target yet — your profile is missing:',
  'planBuilderSafetyAvoid': 'Avoid',
  'planBuilderSafetyCaution': 'Caution',
  'planBuilderSafetyModify': 'Modify',
  'planBuilderSafetyMatched': 'matches your',
  'planBuilderMissingKit': 'Needs kit not on your profile:',
  'planBuilderAllergenWarning': 'Not confirmed free from:',
  'planBuilderAllergenUnverifiable': 'The library holds no information either way on:',
  'planBuilderOffDiet': 'Outside your diet:',
  'planBuilderDuplicateButton': 'Copy this day…',

  // ── Swap ──────────────────────────────────────────────────────────────
  // "Same again, but not that one." Three decisions in the order someone
  // actually makes them: what instead, where, and what happens to the numbers.
  'swapSheetTitle': 'Swap for something else',
  'swapSheetCancel': 'Cancel',
  'swapSheetInsteadHeading': 'Instead of this',
  'swapSheetLoading': 'Finding alternatives…',
  'swapSheetFailed': "Couldn't load alternatives — check your connection.",
  'swapSheetNoAlternatives': 'Nothing in the library does quite the same job.',
  'swapSheetWhereHeading': 'Change it where?',
  'swapSheetSelectRemaining': 'All remaining',
  'swapSheetDayWord': 'Day',
  'swapSheetDoneDaysNote': 'Days you have already finished are ticked and left out by default.',
  'swapSheetNumbersHeading': 'Your sets and reps',
  'swapSheetOverridden': 'Your choice — overriding the suggestion.',
  'swapSheetKeepNumbers': 'Keep all',
  'swapSheetKeepRepsOnly': 'Keep reps',
  'swapSheetStartFresh': 'Start fresh',
  'swapSheetPickOne': 'Pick a replacement',
  'swapSheetSwapButton': 'Swap',
  'swapSheetApplying': 'Swapping…',

  // ── Duplicate ─────────────────────────────────────────────────────────
  'duplicateSheetTitle': 'Copy',
  'duplicateSheetCancel': 'Cancel',
  'duplicateSheetDayWord': 'Day',
  'duplicateSheetWeekWord': 'Week',
  'duplicateSheetModeDay': 'This day',
  'duplicateSheetModeWeek': 'This whole week',
  'duplicateSheetOntoDays': 'Copy onto which days?',
  'duplicateSheetOntoWeek': 'Copy onto which week?',
  'duplicateSheetKeepsNote': 'Copies the training, meals, title and notes. It does not copy anything you have already completed — a copied day starts unfinished.',
  'duplicateSheetOverwriteOne': 'One day already has something on it and will be replaced.',
  'duplicateSheetOverwriteManyPrefix': 'Replaces what is already on',
  'duplicateSheetOverwriteManySuffix': 'days.',
  'duplicateSheetPickTarget': 'Choose where to copy to',
  'duplicateSheetCopyButton': 'Copy',
  'duplicateSheetProgressHeading': 'Make the copy harder?',
  'duplicateSheetProgressSame': 'Same again',
  'duplicateSheetProgressRep': 'One more rep',
  'duplicateSheetProgressSet': 'One more set',
  'duplicateSheetProgressSameNote': 'An identical week repeated for a month stops working. A small step up is usually the right call.',
  'duplicateSheetProgressNote': 'Applied to the copy only, and never to warm-ups, cool-downs or mobility work. Weights are left alone — only you know what you actually lifted.',

  // ── Ask Ava (per-day assist) ──────────────────────────────────────────
  // You drive; Ava helps with the parts you want. It proposes and never
  // saves until you accept, so the copy can be inviting rather than cautious.
  'planBuilderAskAva': 'Ask Ava about this day',
  'planBuilderAskAvaEmpty': 'Ask Ava to fill this day',
  'assistTitle': 'Ask Ava',
  'assistDayWord': 'Day',
  'assistClose': 'Close',
  'assistPlaceholder': 'What would you like help with? For example: swap the squat for something kinder on my knee.',
  'assistKeepsYourWork': 'Ava works around what you have already put here — nothing is saved until you accept it.',
  'assistPromptFill': 'Fill this day for me',
  'assistPromptFinisher': 'Add a finisher',
  'assistPromptWarmup': 'Add a warm-up',
  'assistPromptEasier': 'Make this session easier',
  'assistPromptProtein': 'More protein today',
  'assistPromptQuicker': 'Quicker meals',
  'assistThinking': 'Ava is working on this day…',
  'assistThinkingHint': 'This takes about a minute — she is checking it against your profile and the rest of your week.',
  'assistThinkingShort': 'Working…',
  'assistAsk': 'Ask Ava',
  'assistFailed': "That didn't work — try again in a moment.",
  'assistNeedsAccount': 'Sign in to your Ava account to use this.',
  'assistAvaSays': 'Ava',
  'assistNoNote': 'Here is the day.',
  'assistUnverifiable': 'The library holds no information either way on:',
  'assistWhatChanged': 'What changed',
  'assistNothingChanged': 'Nothing changed.',
  'assistUnchangedSuffix': 'left as they were',
  'assistDiscard': 'Discard',
  'assistUseThis': 'Use this',

  // ── Repeat, progressed ────────────────────────────────────────────────
  // Repeating used to re-run a plan identically, which is the one thing a
  // training programme must not do. It now advances from the log, and shows
  // its reasoning before changing the weight on anyone's bar.
  'plansCreateCustomDays': 'or days',
  'plansCreateDurationHint': 'Short plans get finished. Repeat one when it is done and it moves up from what you actually logged.',
  'repeatSheetTitle': 'Repeat, progressed',
  'repeatSheetSame': 'same',
  'repeatSheetNoEvidence': 'Nothing was logged against this plan, so it repeats exactly as it was. Log your sets next time and the repeat will move you on from what you actually did.',
  'repeatSheetCancel': 'Cancel',
  'repeatSheetStart': 'Start it',

  // ── Week strip ────────────────────────────────────────────────────────
  // Today used to be hardcoded to today, so a meal you forgot to tick was lost.
  'weekStripBackToToday': 'Back to today',
  // Plan cards say where you are in a programme, not "tap to build".
  'plansCardDayWord': 'day',
  'plansCardKept': 'kept',
  'plansCardToday': 'Today:',
  // A rest day is a prescription, not an absence — a blank card reads as
  // "the plan forgot about today" and invites training through it.
  'todayRestDayTitle': 'Rest day',
  'planBuilderRestDayHint': 'No training today by design. Switch this day to Training above if you want to add some.',
  'todayLogDone': 'Done',
  // The calendar shows what HAPPENED, not just where a plan was placed —
  // a month of kept training used to look identical to a month of intentions.
  'plansCalendarLegendDone': 'Done',
  'plansCalendarLegendMissed': 'Missed',
  // A session has a shape a coach would recognise. The library records which
  // job each exercise does; one flat list threw that away.
  'sessionGroupWarmup': 'Warm-up',
  'sessionGroupMain': 'Main lifts',
  'sessionGroupAccessory': 'Accessory',
  'sessionGroupFinisher': 'Finisher',
  'sessionGroupCooldown': 'Cool-down',

  // ── Two doors: ask Ava, or build it yourself ──────────────────────────
  // There used to be one button, "New", which meant manual — so there was no
  // separation between the two, because there was no visible choice at all.
  // Starter plans — the cold-start door. Free, professionally built.
  'startersTitle': 'Starter plans',
  'startersSubtitle': 'Professionally built. Free to begin.',
  'startersDoorButton': 'Start from a plan',
  'startersDoorHint': 'A good week, ready to go — no credits, nothing to build',
  'startersLoading': 'Loading plans…',
  'startersFailed': 'Could not reach the plan library. Try again in a moment.',
  'startersEmpty': 'No starter plans published yet.',
  'startersStartButton': 'Start this plan',
  'startersStarting': 'Starting…',
  'startersSingleSession': 'Single session',
  'startersOneWeek': '1 week',
  'startersDays': 'days',
  'startersDay': 'Day',
  'startersRest': 'Rest',
  'startersSessions': 'sessions',
  'startersRestDays': 'rest days',
  'startersMinutes': 'min a session',
  'startersExercises': 'exercises',
  'startersEquipment': 'What you need',

  'plansAskAvaButton': 'Ask Ava',
  'plansBuildOwnButton': 'Build my own',
  'plansAskAvaHint': 'She writes the whole thing',
  'plansBuildOwnHint': 'Blank days, you fill them',
  'generateTitle': 'Ask Ava for a plan',
  'generateSubtitle': 'She builds it from your profile and your library.',
  'generateGoalLabel': 'Anything in particular?',
  'generateGoalPlaceholder': 'Optional — e.g. more Mediterranean food, easier on my knee',
  'generateKnowsHeading': 'What she is working from',
  'generateKnowsEmpty': 'Your profile is empty, so this will be generic. Fill in your goal, injuries and allergens and she can actually tailor it.',
  'generateKnowsGoal': 'Goal',
  'generateKnowsTraining': 'Training',
  'generateKnowsInjuries': 'Avoiding',
  'generateKnowsAllergens': 'Allergic to',
  'generateKnowsDiet': 'Diet',
  'generateKnowsCooking': 'Weekday cooking',
  'generateKnowsTarget': 'Daily target',
  'generateGo': 'Build it',
  'generateCreditsWord': 'credits',
  'generateNeedsAccount': 'Sign in to your Ava account to use this.',
  'generateFailed': "That didn't work — try again in a moment.",
  'generateWorkingTitle': 'Building your plan',
  'generateWorkingLine': 'Ava is writing it now.',
  'generateWorkingHint': 'She is choosing from a library already filtered for your injuries, allergens and the time you have to cook. This takes a minute or two.',
  'generateDaysWord': 'days',
  'generateDayWord': 'Day',
  'generateRestWord': 'Rest',
  'generateStatSessions': 'sessions',
  'generateStatMeals': 'meals',
  'generateStatCredits': 'credits',
  'generateFilteredHeading': 'Kept out of your plan',
  'generateRemovedAllergen': 'recipes with your allergens',
  'generateRemovedUnsafe': 'exercises against your injuries',
  'generateRemovedSlow': 'recipes that take too long',
  'generateRemovedOffDiet': 'recipes outside your diet',
  'generateRemovedNoKit': 'exercises needing kit you have not got',
  'generateUnverifiable': 'The library holds no information either way on:',
  'generateEditableHint': 'Nothing is fixed — swap anything, or ask Ava to change a day, once it is saved.',
  'generateAgain': 'Try again',
  // The empty state points AT the two doors above rather than carrying a third
  // button that silently meant "manual".
  'plansEmptyStateHint': 'Ask Ava to write one, or build your own — both up top.',
  // A plan row names a thing and prescribes numbers. The technique guide and
  // the recipe were sitting behind the slug the row already carried.
  'mealSheetViewRecipe': 'View recipe',
  // Only an ACTIVE plan gets a date, and only a dated plan appears on the
  // calendar or in Today. Saving a draft silently made the plan unfindable.
  'generateStartToday': 'Start it today',
  'generateSaveDraft': 'Save for later',
  // Active recovery is not a lighter training day — it is a deliberate day of
  // easy movement, and the builder was rendering it identically to training.
  'planBuilderRecoverySection': 'Easy movement',
  'planBuilderRecoveryHint': 'A deliberate easy day — mobility, a walk, some stretching. Keep it light; that is what makes the hard days work.',

  // ── Catalogue (browse + detail) ───────────────────────────────────────
  'catalogBrowseLoadError': "Couldn't load — tap to retry.",
  'catalogWorkoutsTitle': 'Workouts',
  'catalogWorkoutsSearchPlaceholder': 'Search workouts…',
  'catalogRecipesTitle': 'Recipes',
  'catalogRecipesSearchPlaceholder': 'Search recipes…',
  'catalogFilterAll': 'All',
  'catalogBrowseEmptyState': 'Nothing here yet.',
  'catalogBrowseLoadMore': 'Load more',
  'catalogDetailBackButton': 'Back',
  'catalogDetailExerciseLoadError': "Couldn't load this exercise.",
  'catalogDetailRecipeLoadError': "Couldn't load this recipe.",
  'catalogDetailMusclesSection': 'Muscles',
  'catalogDetailTakeCareSection': 'Take care with',
  'catalogDetailCuesSection': 'Cues',
  'catalogDetailCardioSection': 'Cardio',
  'catalogDetailAlternativesSection': 'Easier, harder, instead',
  'catalogDetailEasierLabel': 'Easier',
  'catalogDetailHarderLabel': 'Harder',
  'catalogDetailInsteadLabel': 'Instead',
  'catalogDetailJustForLevel': 'Just for',
  'catalogDetailEquipmentSection': 'Equipment',
  'catalogDetailRoutineSection': 'Routine',
  'catalogDetailProgressionLabel': 'Progression:',
  'catalogDetailHowToSection': 'How to',
  'catalogDetailCommonMistakesSection': 'Common mistakes',
  'catalogDetailWatchDemoButton': 'Watch demo →',
  'catalogDetailNutritionSection': 'Nutrition (per serving)',
  'catalogDetailIngredientsSection': 'Ingredients',
  'catalogDetailIngredientOptional': '(optional)',
  'catalogDetailMethodSection': 'Method',
  'catalogDetailTrickyFlag': 'tricky',
  'catalogDetailSourceLabel': 'Source:',

  // ── Backup & Transfer (.ava-backup export/import) ─────────────────────
  'backupSection': 'Backup & Transfer',
  'backupIntro': "Make a backup file, or move your data here from the desktop app. It's the same file the IDE and extension use — tasks, journal, health and personality load straight in; anything else is carried along untouched.",
  'backupExportEnc': 'Export encrypted backup',
  'backupExportReadable': 'Export readable copy',
  'backupImport': 'Import a backup file',
  'backupImportHint': "Existing data is kept — importing only adds what's new.",
  'backupSavedEnc': "Encrypted backup saved. Keep your passphrase safe — without it the file can't be opened.",
  'backupSavedReadable': 'Readable export saved. Anyone with this file can read it — keep it private.',
  'backupImported': 'Import complete — {n} items carried over.',
  'backupImportedSkipped': ' {n} were already here and left unchanged.',
  'backupImportedEmpty': 'Nothing new to import.',
  'backupErrExport': 'Could not create the backup.',
  'backupErrImport': 'Could not import that file.',
  'backupChoosePass': 'Choose a passphrase',
  'backupEnterPass': 'Enter the passphrase',
  'backupChoosePassDesc': "This locks the backup file. You'll need the exact passphrase to open it on any device — there's no recovery.",
  'backupEnterPassDesc': 'This file is encrypted. Enter the passphrase it was created with.',
  'backupPassphrase': 'Passphrase',
  'backupCreate': 'Create backup',
  'backupImportAction': 'Import',
  'backupWorking': 'Working…',

  // ── Onboarding (shared flow) ────────────────────────────────────────
  // Copied from packages/core/src/i18n/locales/en.ts — the companion
  // reads its own locales, so the shared onboarding flow's keys must
  // exist here too. Keep in step when core's onboarding copy changes.
  'onboarding.identity.title': "Hi, I'm Ava.",
  'onboarding.identity.body': "A living assistant who works alongside you — to build, write, learn, create and look after the everyday. Let's make this yours.",
  'onboarding.identity.cta': "Let's begin",
  'onboarding.path.title': 'What brings you to Ava?',
  'onboarding.path.subtitle': "Pick what fits today — you'll see everything else too. Nothing's locked.",
  'onboarding.path.build.label': 'Build / Code',
  'onboarding.path.build.blurb': 'Ship features, plan architecture, audit for security.',
  'onboarding.path.write.label': 'Write',
  'onboarding.path.write.blurb': 'Draft, edit and shape documents — not code.',
  'onboarding.path.learn.label': 'Learn',
  'onboarding.path.learn.blurb': 'Learn anything, at your pace, with a tutor who quizzes you.',
  'onboarding.path.wellbeing.label': 'Wellbeing',
  'onboarding.path.wellbeing.blurb': 'Track fitness, plan meals, check in on how you are.',
  'onboarding.path.reflect.label': 'Reflect',
  'onboarding.path.reflect.blurb': 'Journal, remember what matters, think out loud.',
  'onboarding.path.explore.label': 'Just exploring',
  'onboarding.path.explore.blurb': 'Show me a little of everything.',
  'onboarding.tailored.heading': "Here's what that looks like",
  'onboarding.tailored.build.ship.title': 'Ship code',
  'onboarding.tailored.build.ship.example': 'add a cancel button to the upload form',
  'onboarding.tailored.build.plan.title': 'Think it through first',
  'onboarding.tailored.build.plan.example': 'should I split this into a service?',
  'onboarding.tailored.build.audit.title': 'Stay secure',
  'onboarding.tailored.build.audit.example': 'audit this API for injection risks',
  'onboarding.tailored.write.draft.title': 'Draft anything',
  'onboarding.tailored.write.draft.example': 'draft a project proposal for a new app',
  'onboarding.tailored.write.shape.title': 'Polish it into a real document',
  'onboarding.tailored.write.shape.example': 'turn these notes into a one-pager',
  'onboarding.tailored.write.brainstorm.title': 'Find the angle',
  'onboarding.tailored.write.brainstorm.example': 'what should this essay actually argue?',
  'onboarding.tailored.learn.curriculum.title': 'A tutor, not a textbook',
  'onboarding.tailored.learn.curriculum.example': 'teach me Spanish from zero',
  'onboarding.tailored.learn.pace.title': 'At your pace',
  'onboarding.tailored.learn.pace.example': 'explain this like I have ten minutes',
  'onboarding.tailored.learn.quiz.title': 'Make it stick',
  'onboarding.tailored.learn.quiz.example': 'quiz me on what we just covered',
  'onboarding.tailored.wellbeing.fitness.title': 'Move how you want',
  'onboarding.tailored.wellbeing.fitness.example': 'a 20-minute home workout, no equipment',
  'onboarding.tailored.wellbeing.recipes.title': 'Eat well',
  'onboarding.tailored.wellbeing.recipes.example': 'a week of easy high-protein dinners',
  'onboarding.tailored.wellbeing.checkin.title': 'Check in',
  'onboarding.tailored.wellbeing.checkin.example': "I'm low on energy lately — any ideas?",
  'onboarding.tailored.reflect.journal.title': 'A journal that listens',
  'onboarding.tailored.reflect.journal.example': 'help me write today down',
  'onboarding.tailored.reflect.remember.title': 'Remembers what matters',
  'onboarding.tailored.reflect.remember.example': 'what was on my mind last week?',
  'onboarding.tailored.reflect.think.title': 'Think out loud',
  'onboarding.tailored.reflect.think.example': "I'm weighing up a big decision",
  'onboarding.tailored.explore.everything.title': 'A bit of everything',
  'onboarding.tailored.explore.everything.example': 'what can you actually do?',
  'onboarding.tailored.explore.modes.title': 'Seven mindsets',
  'onboarding.tailored.explore.modes.example': 'show me the difference between modes',
  'onboarding.tailored.explore.yours.title': 'Yours to shape',
  'onboarding.tailored.explore.yours.example': 'remember that I prefer short answers',
  'onboarding.breadth.title': "And there's so much more I can do.",
  'onboarding.breadth.subtitle': 'Whatever you came for, the rest is here when you want it.',
  'onboarding.breadth.group.modes': 'Seven mindsets',
  'onboarding.breadth.group.everyday': 'Your everyday',
  'onboarding.breadth.group.create': 'Create',
  'onboarding.breadth.group.stays': 'Stays with you',
  'onboarding.breadth.group.desktop': 'On your desktop',
  'onboarding.feature.health.label': 'Health & fitness',
  'onboarding.feature.health.blurb': 'Workouts and plans that fit your life.',
  'onboarding.feature.recipes.label': 'Recipes & meals',
  'onboarding.feature.recipes.blurb': 'Meal plans and recipes for how you eat.',
  'onboarding.feature.learning.label': 'Learning paths',
  'onboarding.feature.learning.blurb': 'Interactive lessons that grade your answers.',
  'onboarding.feature.journal.label': 'Journal',
  'onboarding.feature.journal.blurb': 'A private, local journal — yours alone.',
  'onboarding.feature.images.label': 'Images',
  'onboarding.feature.images.blurb': 'Generate art and visuals from a prompt.',
  'onboarding.feature.music.label': 'Music',
  'onboarding.feature.music.blurb': 'Compose tracks and soundscapes.',
  'onboarding.feature.video.label': 'Video',
  'onboarding.feature.video.blurb': 'Bring ideas to life in motion.',
  'onboarding.feature.memory.label': 'Memory',
  'onboarding.feature.memory.blurb': 'She learns your preferences over time.',
  'onboarding.feature.tasks.label': 'Tasks',
  'onboarding.feature.tasks.blurb': 'Keep track of what needs doing.',
  'onboarding.feature.privacy.label': 'Private by default',
  'onboarding.feature.privacy.blurb': 'Local-first. Nothing leaves unless you sync it.',
  'onboarding.feature.desktop.label': 'Desktop automation',
  'onboarding.feature.desktop.blurb': 'She can see your screen and drive any app — opt-in.',
  'onboarding.mode.work.label': 'Code',
  'onboarding.mode.work.example': 'add a cancel button to the upload form',
  'onboarding.mode.write.label': 'Write',
  'onboarding.mode.write.example': 'draft a project proposal',
  'onboarding.mode.plan.label': 'Plan',
  'onboarding.mode.plan.example': 'should I extract this into a service?',
  'onboarding.mode.chat.label': 'Chat',
  'onboarding.mode.chat.example': 'how do I feel about this launch date?',
  'onboarding.mode.teach.label': 'Teach',
  'onboarding.mode.teach.example': 'teach me Rust async from zero',
  'onboarding.mode.security.label': 'Security',
  'onboarding.mode.security.example': 'audit this API for injection risks',
  'onboarding.mode.brainstorm.label': 'Brainstorm',
  'onboarding.mode.brainstorm.example': 'what should I build with two free weeks?',
  'onboarding.connect.title': 'Make it yours.',
  'onboarding.connect.subtitle': 'Optional — Ava works fully on your machine without an account.',
  'onboarding.connect.body': 'Sign in for free monthly credits, or bring your own key. Either way, your data stays on your machine unless you choose to sync.',
  'onboarding.connect.skip': 'Maybe later',
  'onboarding.consent.title': 'Before we begin.',
  'onboarding.consent.body': 'Ava is built by Augmented Value Acceleration Ltd.',
  'onboarding.consent.local': 'Your data is stored locally on your machine by default.',
  'onboarding.consent.sync': 'Cloud sync is opt-in only — never automatic.',
  'onboarding.consent.train': 'Your work is never used to train AI models.',
  'onboarding.consent.keys': 'API keys are stored securely and never sent to our servers.',
  'onboarding.consent.track': 'No tracking, no analytics, no ads.',
  'onboarding.consent.agree': 'I have read and agree to the Terms and Privacy Policy.',
  'onboarding.desktop.title': 'Ava has hands here.',
  'onboarding.desktop.body': "In the IDE she can drive your desktop — the one thing the editor extension can't do.",
  'onboarding.desktop.see.title': 'See your screen',
  'onboarding.desktop.see.body': "Reads what's on screen to help in context.",
  'onboarding.desktop.drive.title': 'Drive any app',
  'onboarding.desktop.drive.body': 'Clicks and types like you would.',
  'onboarding.desktop.browse.title': 'Run a browser',
  'onboarding.desktop.browse.body': 'Researches and fills things in for you.',
  'onboarding.desktop.control.title': 'You stay in charge',
  'onboarding.desktop.control.body': 'Off by default. Kill-switch one hotkey away.',
  'onboarding.desktop.note': 'Opt in when you want it, from the chat header.',
  'onboarding.hours.title': 'When do you work?',
  'onboarding.hours.body': "Tell Ava your hours and she'll never nudge you to stop during them.",
  'onboarding.hours.start': 'Start',
  'onboarding.hours.end': 'End',
  'onboarding.hours.note': 'Change this any time from the Command Centre clock.',
  'onboarding.ready.title': "We're ready.",
  'onboarding.ready.body': "I've set you up for {path}. Switch mindset any time with the mode pill.",
  'onboarding.ready.go': "Let's go",
  'onboarding.try_this': 'Try this',
  'onboarding.skip': 'Skip',
  'onboarding.back': 'Back',
  'onboarding.next': 'Next',
  'onboarding.agree': 'I agree',
  'onboarding.step_of': 'Step {current} of {total}',
  'onboarding.replay': 'Replay welcome tour',
  'onboarding.show_on_startup': 'Show on startup',
  'onboarding.ready.docs': 'Docs',
  'onboarding.ready.docs_desc': 'Every mode, every tool',
  'onboarding.ready.create': 'Create',
  'onboarding.ready.create_desc': 'Images, music, video',
  'onboarding.ready.settings': 'Settings',
  'onboarding.ready.settings_desc': 'Account, keys, sync',

  // ── health.browse.* (copied from core/en.ts) ──
  // The companion reads its own locales, so shared keys must exist here
  // too. Keep in step when core's copy changes.
  'health.browse.title': 'Health & Nutrition',
  'health.browse.summary': '{exercises} exercises · {recipes} recipes · free, open library',
  'health.browse.safety_link': 'informational only — read safety policy',
  'health.browse.contribute': '+ Contribute',
  'health.browse.tab.exercises': 'Exercises',
  'health.browse.tab.recipes': 'Recipes',
  'health.browse.tab.plans': 'Plans',
  'health.browse.tab.ava': 'Ava',
  'health.browse.edit_profile': 'Edit your profile →',
  'health.browse.your_plans': 'Your plans →',
  'health.browse.tab.mine': 'My submissions',
  'health.browse.tab.profile': 'Profile',
  'health.browse.workout.strength': 'Strength',
  'health.browse.workout.hypertrophy': 'Hypertrophy',
  'health.browse.workout.conditioning': 'Conditioning',
  'health.browse.workout.mobility': 'Mobility',
  'health.browse.workout.hybrid': 'Hybrid',
  'health.browse.workout.yoga': 'Yoga',
  'health.browse.workout.pilates': 'Pilates',
  'health.browse.workout.running': 'Running',
  'health.browse.workout.cycling': 'Cycling',
  'health.browse.workout.recovery': 'Recovery',
  'health.browse.workout.hiit': 'HIIT',
  'health.browse.course.breakfast': 'Breakfast',
  'health.browse.course.main': 'Mains',
  'health.browse.course.starter': 'Starters',
  'health.browse.course.side': 'Sides',
  'health.browse.course.snack': 'Snacks',
  'health.browse.course.dessert': 'Desserts',
  'health.browse.noun.exercises': 'exercises',
  'health.browse.noun.recipes': 'recipes',
  'health.browse.load_error': 'Couldn\'t load {noun}.',
  'health.browse.load_error_hint': 'The connection to the library failed — your data is safe, this is a network hiccup.',
  'health.browse.retry': 'Retry',
  'health.browse.search_exercises_placeholder': 'Search exercises — name, e.g. \'squat\'',
  'health.browse.search_recipes_placeholder': 'Search recipes — name, e.g. \'chicken\'',
  'health.browse.filter.all': 'All',
  'health.browse.from_scratch': 'From Scratch',
  'health.browse.from_scratch_hint': 'Made entirely from scratch — fresh ingredients, nothing processed',
  'health.browse.no_exercises_match_q': 'No exercises match "{q}".',
  'health.browse.no_exercises_match': 'No exercises match.',
  'health.browse.no_recipes_match_q': 'No recipes match "{q}".',
  'health.browse.no_recipes_match': 'No recipes match.',
  'health.browse.prev': 'Prev',
  'health.browse.next': 'Next',
  'health.browse.page_of': 'Page {current} of {total}',
  'health.browse.loading': 'loading…',
  'health.browse.close': 'Close',
  'health.browse.loading_exercise': 'Loading exercise…',
  'health.browse.loading_recipe': 'Loading recipe…',
  'health.browse.failed_to_load': 'Failed to load.',
  'health.browse.difficulty_n': 'Difficulty {n}/5',
  'health.browse.difficulty_of_5': 'Difficulty {n} of 5',
  'health.browse.routine.sets': 'Sets',
  'health.browse.routine.reps': 'Reps',
  'health.browse.routine.rest': 'Rest',
  'health.browse.routine.tempo': 'Tempo',
  'health.browse.routine.freq': 'Freq.',
  'health.browse.how_to_do_it': 'How to do it',
  'health.browse.how_to_use_it': 'How to use it',
  'health.browse.if_youre_new': 'If you\'re new to this',
  'health.browse.common_mistakes': 'Common mistakes',
  'health.browse.muscles': 'Muscles',
  'health.browse.equipment': 'Equipment',
  'health.browse.ingredients': 'Ingredients',
  'health.browse.opt': '(opt.)',
  'health.browse.method': 'Method',
  'health.browse.overview': 'Overview',
  'health.browse.howto': 'How-to',
  'health.browse.routine': 'Routine',
  'health.browse.storage': 'Storage',
  'health.browse.nutrition': 'Nutrition',
  'health.browse.nothing_here': 'Nothing here yet.',
  'health.browse.no_overview': 'No overview yet.',
  'health.browse.per_serving': 'Per serving',
  'health.browse.per_serving_estimated': 'Per serving · Ava-estimated.',
  'health.browse.nutri.calories': 'Calories',
  'health.browse.nutri.protein': 'Protein',
  'health.browse.nutri.carbs': 'Carbs',
  'health.browse.nutri.fat': 'Fat',
  'health.browse.nutri.sat_fat': 'Sat fat',
  'health.browse.nutri.fibre': 'Fibre',
  'health.browse.nutri.sugar': 'Sugar',
  'health.browse.nutri.sodium': 'Sodium',
  'health.browse.level.beginner': 'beginner',
  'health.browse.level.intermediate': 'intermediate',
  'health.browse.level.expert': 'expert',
  'health.browse.prep': 'Prep',
  'health.browse.cook': 'Cook',
  'health.browse.total': 'Total',
  'health.browse.serves': 'Serves',
  'health.browse.optional': 'optional',
  'health.browse.tricky': 'tricky',
  'health.browse.status.pending': 'Pending review',
  'health.browse.status.rejected': 'Rejected',
  'health.browse.refresh': 'Refresh',
  'health.browse.refreshing': 'Refreshing',
  'health.browse.clear_search': 'Clear search',
  'health.browse.subtitle': 'A free, open library of exercises and recipes. Informational only.',
  'health.browse.loading_exercises': 'Loading exercises…',
  'health.browse.loading_recipes': 'Loading recipes…',
  'health.browse.no_exercises_found': 'No exercises found.',
  'health.browse.no_recipes_found': 'No recipes found.',
  'health.browse.couldnt_load_exercise': 'Couldn\'t load this exercise.',
  'health.browse.couldnt_load_recipe': 'Couldn\'t load this recipe.',
  'health.browse.course.beverage': 'Beverages',
  'health.browse.course.sauce': 'Sauces',
  'health.browse.course.bread': 'Breads',

  // ── news.* (copied from core/en.ts) ──
  // The companion reads its own locales, so shared keys must exist here
  // too. Keep in step when core's copy changes.
  'news.all': 'All',
  'news.breaking': 'Breaking',
  'news.ai_agents': 'AI Agents',
  'news.models': 'Models',
  'news.dev_tools': 'Dev Tools',
  'news.open_source': 'Open Source',
  'news.education': 'Education',
  'news.productivity': 'Productivity',
  'news.companions': 'Companions',
  'news.health': 'Health',
  'news.enterprise': 'Enterprise',
  'news.industry': 'Industry',
  'news.min_read': '{n} min read',
  'news.world': 'World News',
  'news.ai': 'AI',
  'news.technology': 'Technology',
  'news.security_privacy': 'Security & Privacy',
  'news.business': 'Business & Economy',
  'news.science': 'Science',
  'news.food': 'Food & Nutrition',
  'news.sport': 'Sport',
};

export type StringKey = keyof typeof enStrings;
